# AGENTS.md - Project Context for Out of the Books

### Introduction and Core Principles\*\*

I am Codex, an senior software engineer with more than 20 years of experience in software and web development, Phd-level, demonstrating expert skills and following best and modern practices in
...
> ⚠️ **Generated content — do not hand-edit.** `src/content/festival/*.json` and
> `src/content/post/5_FICHES/*.mdx` are **regenerated from NocoDB on every build**
> (`optimize:prebuild` → `build-festival-data.js` / `build-fiches-pedagogiques.js`).
> Hand-edits/commits are overwritten — change the source in NocoDB. (`NOCODB_API_TOKEN`
> must be set on Netlify; the committed files act as a fallback when it's absent.)

### Key Directories

- `src/content/` - Content files (MDX, JSON)
- `src/components/` - Reusable components
- `src/scripts/build-festival-data.js` / `build-fiches-pedagogiques.js` - NocoDB → JSON/MDX build scripts
...
# Data Management (NocoDB → JSON/MDX)
npm run build:events   # Rebuild festival data from NocoDB (--reset)
npm run build:fiches   # Rebuild pedagogical sheets from NocoDB
npm run fetch:festival # Fetch-only festival data (no reset)

# Quality
npm run check       # astro check + eslint + prettier (full quality gate)
...
### TinaCMS Staging Sandbox

`tina/config.ts` resolves the edit branch dynamically (`process.env.TINA_BRANCH || process.env.HEAD || "main"`) for both `branch` and `gitProvider.branch`, with `autoMerge: false`. So the `staging` branch deploy (`staging--outofthebooks.netlify.app/admin`) edits the **`staging`** branch, isolated from production, while the prod deploy still edits `main` (`HEAD=main`). Model: two independent environments — direct prod editing vs throwaway staging sandbox (reset from `main` anytime). Changing the branch _value_ is NOT a schema change → no `tina-lock.json` regen.

### TinaCMS Schema Rule (CRITICAL)

**After ANY schema change in `tina/*.ts` files (add/remove/rename field, collection, or type), regenerate and commit `tina-lock.json`.**

```bash
# Run in a clean shell (no other background jobs).
# Starts dev in background to regenerate tina-lock.json, then kills it.
npx tinacms dev --no-server &
TINA_PID=$!
sleep 15              # increase to 25-30 on slow machines or cold npm cache
kill $TINA_PID 2>/dev/null
wait $TINA_PID 2>/dev/null

# Verify the file was updated, then commit
git diff --stat tina/tina-lock.json   # should show 1 file changed
git add tina/tina-lock.json
git commit -m "fix(tina): regenerate tina-lock.json after schema change"
git push origin HEAD  # adjust if not on main
```

Wait ~35s for TinaCloud to re-index, then verify: `npx tinacms build` should complete without schema errors.

If the mismatch persists, the lock file may not have been fully written — increase `sleep` to 25-30 and retry.

Skipping this causes "Tina Schema Mismatch" errors. TinaCloud reads `tina-lock.json` from git as its schema source — it does NOT re-derive the schema from TypeScript files.

> **Why `dev` not `build`**: `npx tinacms build` aborts at the cloud check when already in a mismatch state. `npx tinacms dev --no-server` regenerates `tina-lock.json` from TypeScript **without** that check.

See `memory-bank/troubleshoot.md` for full diagnostic + emergency patch procedure.
