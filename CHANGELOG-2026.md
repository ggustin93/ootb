...
- **Vérifié** — `npm run check`, les tests utilitaires et les 75 tests des fonctions Netlify passent. `npm run build` passe : les 158 images du festival sont régénérées avec `sharp` 0.35 et vérifiées visuellement. Deux sources restent en image de remplacement : un fichier EPS (stand 1451) et un logo ICO (conférence 640). Le comportement est identique avec l'ancien `sharp` 0.34.5, donc ce n'est pas une régression.
