  ...
  if (event.httpMethod === 'GET') {
    ...
    try {
      ...
      try {
        ...
        const result = await api.dbTableRow.list(NOCODB_ORG_ID, NOCODB_PROJECT_ID, NOCODB_TABLE_ID, { limit: 1 });
  ...
  if (event.httpMethod === 'POST') {
    ...
    try {
      ...
      if (!api) {
        ...
        return {
          ...
          body: JSON.stringify({
            success: false,
            message:
              'Nous rencontrons actuellement un problème technique. Notre équipe a été informée et travaille à résoudre ce problème. Veuillez réessayer ultérieurement.',
            error: 'API NocoDB non initialisée',
          }),
          headers: {
            'Content-Type': 'application/json',
      ...
      try {
        ...
        const result = await api.dbTableRow.create(NOCODB_ORG_ID, NOCODB_PROJECT_ID, NOCODB_TABLE_ID, formattedData);
        ...
        return {
          ...
          body: JSON.stringify({
            success: false,
            message:
              "Nous n'avons pas pu envoyer votre message. Veuillez vérifier votre connexion internet et réessayer. Si le problème persiste, contactez-nous par email.",
            error: errorMessage,
          }),
          headers: {
            'Content-Type': 'application/json',
      ...
      return {
        ...
        body: JSON.stringify({
          success: false,
          message:
            "Une erreur s'est produite lors du traitement de votre demande. Veuillez vérifier les informations saisies et réessayer.",
          error: error.message || 'Erreur inconnue',
        }),
        headers: {
          'Content-Type': 'application/json',
