  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.error(
          `❌ Erreur lors de l'ajout du contact ${email} à la liste:`,
          addError.response?.body || addError.message
        );
...
export const handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    try {
      ...
      if (showRecords) {
        ...
        const records = await api.dbTableRow.list(NOCODB_ORG_ID, NOCODB_PROJECT_ID, NOCODB_NEWSLETTER_TABLE_ID, {
      ...
      const sampleData = await api.dbTableRow.list(NOCODB_ORG_ID, NOCODB_PROJECT_ID, NOCODB_NEWSLETTER_TABLE_ID, {
        limit: 1,
      });
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          data: {
            structure: {
              fields:
                sampleData.list && sampleData.list.length > 0
      ...
      const errorMessage =
        isApiError(error) && error.response?.data?.msg ? error.response.data.msg : error.message || 'Erreur inconnue';
  ...
  if (event.httpMethod === 'POST') {
    ...
    try {
      ...
      try {
        // Vérifier si l'email existe déjà
        const existingSubscribers = await api.dbTableRow.list(orgId, projectId, tableId, {
        ...
        if (existingSubscribers.list && existingSubscribers.list.length > 0) {
          ...
          await api.dbTableRow.update(orgId, projectId, tableId, existingId, {
        ...
        const result = await api.dbTableRow.create(orgId, projectId, tableId, formattedData);
        ...
        return {
          ...
          body: JSON.stringify({
            success: false,
            message:
              "Nous n'avons pas pu enregistrer votre inscription. Veuillez réessayer ou nous contacter si le problème persiste.",
          }),
          headers: {
            'Content-Type': 'application/json',
          },
