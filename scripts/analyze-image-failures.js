...
async function getItemData(api, tableId, itemId) {
  try {
    const response = await api.dbTableRow.list('noco', NOCODB_CONFIG.projectId, tableId, {
      limit: 100,
      offset: 0,
      where: `(ID,eq,${itemId})`,
    });
...
async function analyzeFailures() {
  try {
    ...
    for (const failed of failedIds) {
      ...
      if (
        failed.type !== 'stands' &&
        item['Envoyez une photo de vous'] &&
        Array.isArray(item['Envoyez une photo de vous'])
      ) {
