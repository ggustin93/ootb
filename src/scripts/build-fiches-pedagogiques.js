...
async function fetchFichesPedagogiques() {
  try {
    ...
    const response = await api.dbViewRow.list(NOCODB_ORG_ID, NOCODB_PROJECT_ID, NOCODB_BASE_ID, NOCODB_TABLE_ID, {
      offset: 0,
  ...
  return fiches
    .map((fiche, index) => {
      try {
        ...
        const slug = fiche.Id ? `${edition}-${baseSlug}-${fiche.Id}` : `${edition}-${baseSlug}-${index + 1}`;
        ...
        if (existingDate) {
          ...
          console.log(
            `📅 Fiche "${fiche.Title || 'sans titre'}" (édition ${edition}): date existante conservée: ${publishDate}`
          );
          ...
          console.log(
            `📅 Fiche "${fiche.Title || 'sans titre'}" (édition ${edition}): nouvelle fiche, date courante utilisée: ${publishDate}`
          );
        } else {
          ...
          console.log(
            `📅 Fiche "${fiche.Title || 'sans titre'}" (édition ${edition}): date aléatoire générée: ${publishDate}`
          );
        }
        ...
        if (fiche.Thèmes) {
          try {
            ...
            if (Array.isArray(themes)) {
              ...
              themes.forEach((theme) => {
                if (!shouldIgnoreTheme(theme)) {
                  ...
                  console.log(
                    `⏭️ Ignoré le thème "${theme}" (format 'thème X') pour la fiche "${fiche.Title || 'sans titre'}"`
                  );
                }
              });
              ...
              if (!shouldIgnoreTheme(themes)) {
                ...
                console.log(
                  `⏭️ Ignoré le thème "${themes}" (format 'thème X') pour la fiche "${fiche.Title || 'sans titre'}"`
                );
              }
            }
          } catch {
            ...
            if (!/^thème \d+$/i.test(fiche.Thèmes)) {
              console.log(
                `🏷️ Ajout du thème "${fiche.Thèmes}" (format non-JSON) aux tags pour la fiche "${fiche.Title || 'sans titre'}"`
              );
              tags.push(fiche.Thèmes.toLowerCase());
            } else {
              console.log(
                `⏭️ Ignoré le thème "${fiche.Thèmes}" (format 'thème X') pour la fiche "${fiche.Title || 'sans titre'}"`
              );
            }
          }
        }
        ...
        const objectifs = fiche.Objectifs ? fiche.Objectifs.split('\n').filter((item) => item.trim() !== '') : [];

        // Construire les compétences sous forme de tableau
        const competences = fiche.Competences ? fiche.Competences.split('\n').filter((item) => item.trim() !== '') : [];
...
function cleanSpecificFiches(fichesToRemove) {
  try {
    ...
    const files = fs.readdirSync(FICHES_DIR).filter((file) => file.endsWith('.mdx') && file !== 'README.md');
