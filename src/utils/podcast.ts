...
export function generateAushaPlayer(
    ...
    try {
      ...
      if (urlParts.length >= 4) {
        ...
        if (showSlug === 'education-mode-d-emploi' && !extractedShowId) {
          ...
          console.log(
            "generateAushaPlayer - Utilisation de l'ID par défaut pour education-mode-d-emploi:",
            extractedShowId
          );
        }
  ...
  const finalPodcastId = extractedPodcastId || `EP${Math.random().toString(36).substring(2, 10).toUpperCase()}`;
