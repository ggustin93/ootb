...
import { shouldCompress, compressImage, SKIP_BELOW_BYTES } from '../compressImage.js';
import { ensureUploadable, tooLargeMessage, MAX_UPLOAD_BYTES } from '../uploadGuard.js';
...
function testShouldCompress() {
  ...
  assert(
    !shouldCompress(fakeFile({ type: 'image/jpeg', size: SKIP_BELOW_BYTES })),
    'JPEG pile au seuil → false (strictement >)'
  );
